    ...
    public partial class MediaPlayerElement : ContentView
    {
        ...
        private void PageAppearing(object? sender, Page e)
        {
            if (e == this.FindAncestor<Page?>())
            {
                ...
                {
                    var applicationProperties =  Microsoft.Maui.Storage.Preferences.Default;
                    var mediaPlayer = MediaPlayer;
                    if (mediaPlayer != null)
                    {
                        applicationProperties.Set($"VLC_{mediaPlayer.NativeReference}_MediaPlayerElement_Position", mediaPlayer.Position);
                        applicationProperties.Set($"VLC_{mediaPlayer.NativeReference}_MediaPlayerElement_IsPlaying", mediaPlayer.State == VLCState.Playing);
                        mediaPlayer.Stop();
                    }
                    VideoView = null;
                });
                MessagingCenter.Subscribe<LifecycleMessage>(this, "OnResume", m =>
                {
                    ...
                    {
                        var applicationProperties = Microsoft.Maui.Storage.Preferences.Default;
                        if (applicationProperties.Get<bool>($"VLC_{mediaPlayer.NativeReference}_MediaPlayerElement_IsPlaying", false))
                        {
                            mediaPlayer.Play();
                            mediaPlayer.Position = applicationProperties.Get<float>($"VLC_{mediaPlayer.NativeReference}_MediaPlayerElement_Position", 0.0f);
                        }
                    }
                });
            }
